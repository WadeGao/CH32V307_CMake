/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_rcc.c Author :
 *WCH Version            : V1.0.0 Date               : 2021/06/06 Description :
 *This file provides all the RCC firmware functions. Copyright (c) 2021 Nanjing
...
void RCC_PLLConfig(uint32_t RCC_PLLSource, uint32_t RCC_PLLMul) {
  ...
  if (((*(uint32_t *)0x1FFFF70C) & (1 << 14)) !=
      (1 << 14)) { /* for other CH32V30x */
 ...
 *
 * @param   RCC_SYSCLK - defines the AHB clock divider. This clock is derived
 * from the system clock (SYSCLK). RCC_SYSCLK_Div1 - AHB clock = SYSCLK.
 ...
 *
 * @param   RCC_HCLK - defines the APB1 clock divider. This clock is derived
 * from the AHB clock (HCLK). RCC_HCLK_Div1 - APB1 clock = HCLK. RCC_HCLK_Div2 -
 * APB1 clock = HCLK/2. RCC_HCLK_Div4 - APB1 clock = HCLK/4. RCC_HCLK_Div8 -
 ...
 *
 * @param   RCC_HCLK - defines the APB2 clock divider. This clock is derived
 * from the AHB clock (HCLK). RCC_HCLK_Div1 - APB1 clock = HCLK. RCC_HCLK_Div2 -
 * APB1 clock = HCLK/2. RCC_HCLK_Div4 - APB1 clock = HCLK/4. RCC_HCLK_Div8 -
 * APB1 clock = HCLK/8. RCC_HCLK_Div16 - APB1 clock = HCLK/16.
 *
 ...
 * disabled. RCC_IT_LSIRDY - LSI ready interrupt. RCC_IT_LSERDY - LSE ready
 * interrupt. RCC_IT_HSIRDY - HSI ready interrupt. RCC_IT_HSERDY - HSE ready
 * interrupt. RCC_IT_PLLRDY - PLL ready interrupt. NewState - ENABLE or DISABLE.
 *
 * @return  none
 */
 ...
 *
 * @param   RCC_PCLK2 - defines the ADC clock divider. This clock is derived
 * from the APB2 clock (PCLK2). RCC_PCLK2_Div2 - ADC clock = PCLK2/2.
 ...
 * @fn      RCC_RTCCLKConfig
 *
 * @brief   Once the RTC clock is selected it can't be changed unless the Backup
 * domain is reset.
 *
 * @param   RCC_RTCCLKSource - specifies the RTC clock source.
 *            RCC_RTCCLKSource_LSE - LSE selected as RTC clock.
 *            RCC_RTCCLKSource_LSI - LSI selected as RTC clock.
 *            RCC_RTCCLKSource_HSE_Div128 - HSE clock divided by 128 selected as
 * RTC clock.
 *
 * @return  none
 */
 ...
 *          RCC_I2S2CLKSource_PLL3_VCO - PLL3 VCO clock selected as I2S2 clock
 * entry
 *
 * @return  none
 */
 ...
 *            RCC_I2S3CLKSource_SYSCLK - system clock selected as I2S3 clock
 * entry RCC_I2S3CLKSource_PLL3_VCO - PLL3 VCO clock selected as I2S3 clock
 * entry
 *
 * @return  none
 */
 ...
 *            RCC_RNGCLKSource_PLL3_VCO - PLL3 VCO clock selected as RNG clock
 * entry
 *
 * @return  none
 */
 ...
 *            RCC_ETH1GCLKSource_PLL2_VCO - system clock selected as ETH1G clock
 * entry RCC_ETH1GCLKSource_PLL3_VCO - PLL3 VCO clock selected as ETH1G clock
 * entry RCC_ETH1GCLKSource_PB1_IN -  GPIO PB1 input clock selected as ETH1G
 * clock entry
 *
 * @return  none
 */
 ...
 *            RCC_HSBHSPLLCLKSource_HSE - HSE clock selected as USBHSPLL clock
 * entry RCC_HSBHSPLLCLKSource_HSI - HSI clock selected as USBHSPLL clock entry
 *
 * @return  none
 */
 ...
 *            RCC_USBCLK48MCLKSource_PLLCLK - PLLCLK clock selected as USB clock
 * 48MHz clock entry RCC_USBCLK48MCLKSource_USBPHY - USBPHY clock selected as
 * USB clock 48MHz clock entry
 *
 * @return  none
 */
