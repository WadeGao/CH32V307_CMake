/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_flash.c Author
 *: WCH Version            : V1.0.0 Date               : 2021/06/06 Description
 *: This file provides all the FLASH firmware functions. Copyright (c) 2021
 ...
 * @return  none
 */
void FLASH_Lock(void) { FLASH->CTLR |= CR_LOCK_Set; }
 ...
 * @return  none
 */
void FLASH_LockBank1(void) { FLASH->CTLR |= CR_LOCK_Set; }
 ...
 *
 * @param   FLASH_Sectors - specifies the address of the pages to be write
 * protected.
 *
 * @return  FLASH Status - The returned value can be: FLASH_BUSY,
 * FLASH_ERROR_PG, FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
 ...
 *        and RST_STDBY(Bit2).
 */
uint32_t FLASH_GetUserOptionByte(void) { return (uint32_t)(FLASH->OBR >> 2); }
 ...
 * disabled. FLASH_IT_ERROR - FLASH Error Interrupt FLASH_IT_EOP - FLASH end of
 * operation Interrupt NewState - new state of the specified Flash
 ...
 * @return  none
 */
void FLASH_ClearFlag(uint32_t FLASH_FLAG) { FLASH->STATR = FLASH_FLAG; }
 ...
 * @return  none
 */
void FLASH_Lock_Fast(void) { FLASH->CTLR |= CR_LOCK_Set; }
