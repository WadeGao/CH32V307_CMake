/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_can.c Author :
 *WCH Version            : V1.0.0 Date               : 2021/06/06 Description :
 *This file provides all the CAN firmware functions. Copyright (c) 2021 Nanjing
...
uint8_t CAN_Init(CAN_TypeDef *CANx, CAN_InitTypeDef *CAN_InitStruct) {
  ...
  while (((CANx->STATR & CAN_STATR_INAK) != CAN_STATR_INAK) &&
...
void CAN_Receive(CAN_TypeDef *CANx, uint8_t FIFONumber, CanRxMsg *RxMessage) {
  ...
  if (RxMessage->IDE == CAN_Id_Standard) {
    RxMessage->StdId =
        (uint32_t)0x000007FF & (CANx->sFIFOMailBox[FIFONumber].RXMIR >> 21);
  } else {
    RxMessage->ExtId =
        (uint32_t)0x1FFFFFFF & (CANx->sFIFOMailBox[FIFONumber].RXMIR >> 3);
  }
  ...
  RxMessage->Data[1] =
      (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RXMDLR >> 8);
  RxMessage->Data[2] =
      (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RXMDLR >> 16);
  RxMessage->Data[3] =
      (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RXMDLR >> 24);
  RxMessage->Data[4] = (uint8_t)0xFF & CANx->sFIFOMailBox[FIFONumber].RXMDHR;
  RxMessage->Data[5] =
      (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RXMDHR >> 8);
  RxMessage->Data[6] =
      (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RXMDHR >> 16);
  RxMessage->Data[7] =
      (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RXMDHR >> 24);
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef *CANx, uint8_t CAN_OperatingMode) {
  ...
  if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
    CANx->CTLR =
        (uint32_t)((CANx->CTLR & (uint32_t)(~(uint32_t)CAN_CTLR_SLEEP)) |
                   CAN_CTLR_INRQ);

    while (((CANx->STATR & CAN_MODE_MASK) != CAN_STATR_INAK) &&
           (timeout != 0)) {
      timeout--;
    }
    ...
    CANx->CTLR =
        (uint32_t)((CANx->CTLR & (uint32_t)(~(uint32_t)CAN_CTLR_INRQ)) |
                   CAN_CTLR_SLEEP);

    while (((CANx->STATR & CAN_MODE_MASK) != CAN_STATR_SLAK) &&
           (timeout != 0)) {
      timeout--;
    }
...
uint8_t CAN_Sleep(CAN_TypeDef *CANx) {
  ...
  CANx->CTLR =
      (((CANx->CTLR) & (uint32_t)(~(uint32_t)CAN_CTLR_INRQ)) | CAN_CTLR_SLEEP);
...
uint8_t CAN_WakeUp(CAN_TypeDef *CANx) {
  ...
  while (((CANx->STATR & CAN_STATR_SLAK) == CAN_STATR_SLAK) &&
         (wait_slak != 0x00)) {
    wait_slak--;
  }
 ...
 * disabled. CAN_IT_TME. CAN_IT_FMP0. CAN_IT_FF0. CAN_IT_FOV0. CAN_IT_FMP1.
 *            CAN_IT_FF1.
 *            CAN_IT_FOV1.
 *            CAN_IT_EWG.
 *            CAN_IT_EPV.
...
ITStatus CAN_GetITStatus(CAN_TypeDef *CANx, uint32_t CAN_IT) {
  ...
  if ((CANx->INTENR & CAN_IT) != RESET) {
    switch (CAN_IT) {
      case CAN_IT_TME:
        itstatus =
            CheckITStatus(CANx->TSTATR, CAN_TSTATR_RQCP0 | CAN_TSTATR_RQCP1 |
                                            CAN_TSTATR_RQCP2);
        break;
