...
void ETH_StructInit(ETH_InitTypeDef *ETH_InitStruct) {
  ...
  ETH_InitStruct->ETH_ReceiveThresholdControl =
      ETH_ReceiveThresholdControl_64Bytes;
...
uint32_t ETH_HandleTxPkt(uint8_t *ppkt, uint16_t FrameLength) {
  ...
  for (offset = 0; offset < FrameLength; offset++) {
    (*(__IO uint8_t *)((DMATxDescToSet->Buffer1Addr) + offset)) =
        (*(ppkt + offset));
  }
  ...
  if ((DMATxDescToSet->Status & ETH_DMATxDesc_TCH) != (uint32_t)RESET) {
    ...
    if ((DMATxDescToSet->Status & ETH_DMATxDesc_TER) != (uint32_t)RESET) {
      ...
      DMATxDescToSet =
          (ETH_DMADESCTypeDef *)((uint32_t)DMATxDescToSet + 0x10 +
                                 ((ETH->DMABMR & ETH_DMABMR_DSL) >> 2));
    }
  }
...
uint32_t ETH_HandleRxPkt(uint8_t *ppkt) {
  ...
  if ((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RCH) !=
      (uint32_t)RESET) {
    DMARxDescToGet =
        (ETH_DMADESCTypeDef *)(DMARxDescToGet->Buffer2NextDescAddr);
  } else {
    if ((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RER) !=
        (uint32_t)RESET) {
      DMARxDescToGet = (ETH_DMADESCTypeDef *)(ETH->DMARDLAR);
    } else {
      DMARxDescToGet =
          (ETH_DMADESCTypeDef *)((uint32_t)DMARxDescToGet + 0x10 +
                                 ((ETH->DMABMR & ETH_DMABMR_DSL) >> 2));
    }
  }
 ...
 * @return  none
 */
void ETH_InitiatePauseControlFrame(void) { ETH->MACFCR |= ETH_MACFCR_FCBBPA; }
 ...
 * @return  none
 */
void ETH_MACAddressPerfectFilterCmd(uint32_t MacAddr,
                                    FunctionalState NewState) {
  if (NewState != DISABLE) {
    (*(__IO uint32_t *)(ETH_MAC_ADDR_HBASE + MacAddr)) |= ETH_MACA1HR_AE;
  } else {
    (*(__IO uint32_t *)(ETH_MAC_ADDR_HBASE + MacAddr)) &=
        (~(uint32_t)ETH_MACA1HR_AE);
  }
}
...
void ETH_DMATxDescRingInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t *TxBuff1,
  ...
  for (i = 0; i < TxBuffCount; i++) {
    ...
    DMATxDesc->Buffer2NextDescAddr =
        (uint32_t)(&TxBuff2[i * ETH_MAX_PACKET_SIZE]);
...
uint32_t ETH_GetDMATxDescCollisionCount(ETH_DMADESCTypeDef *DMATxDesc) {
  return ((DMATxDesc->Status & ETH_DMATxDesc_CC) >>
...
void ETH_DMATxDescBufferSizeConfig(ETH_DMADESCTypeDef *DMATxDesc,
  ...
  DMATxDesc->ControlBufferSize |=
      (BufferSize1 | (BufferSize2 << ETH_DMATXDESC_BUFFER2_SIZESHIFT));
}
...
void ETH_DMARxDescChainInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff,
  ...
  for (i = 0; i < RxBuffCount; i++) {
    ...
    DMARxDesc->ControlBufferSize =
        ETH_DMARxDesc_RCH | (uint32_t)ETH_MAX_PACKET_SIZE;
...
void ETH_DMARxDescRingInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff1,
  ...
  for (i = 0; i < RxBuffCount; i++) {
    ...
    DMARxDesc->Buffer2NextDescAddr =
        (uint32_t)(&RxBuff2[i * ETH_MAX_PACKET_SIZE]);
...
uint32_t ETH_GetDMARxDescFrameLength(ETH_DMADESCTypeDef *DMARxDesc) {
  return ((DMARxDesc->Status & ETH_DMARxDesc_FL) >>
          ETH_DMARXDESC_FRAME_LENGTHSHIFT);
}
...
uint32_t ETH_GetDMARxDescBufferSize(ETH_DMADESCTypeDef *DMARxDesc,
  ...
  if (DMARxDesc_Buffer != ETH_DMARxDesc_Buffer1) {
    return ((DMARxDesc->ControlBufferSize & ETH_DMARxDesc_RBS2) >>
 ...
 * @return  none
 */
void ETH_SoftwareReset(void) { ETH->DMABMR |= ETH_DMABMR_SR; }
 ...
 * @return  none
 */
void ETH_FlushTransmitFIFO(void) { ETH->DMAOMR |= ETH_DMAOMR_FTF; }
...
uint32_t ETH_GetRxOverflowMissedFrameCounter(void) {
  return ((uint32_t)((ETH->DMAMFBOCR & ETH_DMAMFBOCR_MFA) >>
 ...
 * @return  none
 */
void ETH_ResumeDMATransmission(void) { ETH->DMATPDR = 0; }
 ...
 * @return  none
 */
void ETH_ResumeDMAReception(void) { ETH->DMARPDR = 0; }
 ...
 * @return  none
 */
void ETH_MMCCountersReset(void) { ETH->MMCCR |= ETH_MMCCR_CR; }
...
ITStatus ETH_GetMMCITStatus(uint32_t ETH_MMC_IT) {
  ...
  if ((ETH_MMC_IT & (uint32_t)0x10000000) != (uint32_t)RESET) {
    if ((((ETH->MMCRIR & ETH_MMC_IT) != (uint32_t)RESET)) &&
        ((ETH->MMCRIMR & ETH_MMC_IT) != (uint32_t)RESET)) {
      bitstatus = SET;
    } else {
      bitstatus = RESET;
    }
  } else {
    if ((((ETH->MMCTIR & ETH_MMC_IT) != (uint32_t)RESET)) &&
        ((ETH->MMCRIMR & ETH_MMC_IT) != (uint32_t)RESET)) {
      bitstatus = SET;
    } else {
      bitstatus = RESET;
 ...
 * @return  none
 */
void ETH_EnablePTPTimeStampAddend(void) { ETH->PTPTSCR |= ETH_PTPTSCR_TSARU; }
 ...
 * @return  none
 */
void ETH_InitializePTPTimeStamp(void) { ETH->PTPTSCR |= ETH_PTPTSCR_TSSTI; }
...
uint32_t ETH_HandlePTPTxPkt(uint8_t *ppkt, uint16_t FrameLength,
  ...
  for (offset = 0; offset < FrameLength; offset++) {
    (*(__IO uint8_t *)((DMAPTPTxDescToSet->Buffer1Addr) + offset)) =
        (*(ppkt + offset));
  }
  ...
  do {
    timeout++;
  } while (!(DMATxDescToSet->Status & ETH_DMATxDesc_TTSS) &&
           (timeout < 0xFFFF));
  ...
  *PTPTxTab = DMATxDescToSet->Buffer2NextDescAddr;
  ...
  if ((DMATxDescToSet->Status & ETH_DMATxDesc_TCH) != (uint32_t)RESET) {
    DMATxDescToSet =
        (ETH_DMADESCTypeDef *)(DMAPTPTxDescToSet->Buffer2NextDescAddr);
    ...
    if ((DMATxDescToSet->Status & ETH_DMATxDesc_TER) != (uint32_t)RESET) {
      ...
      DMATxDescToSet =
          (ETH_DMADESCTypeDef *)((uint32_t)DMATxDescToSet + 0x10 +
                                 ((ETH->DMABMR & ETH_DMABMR_DSL) >> 2));
      DMAPTPTxDescToSet =
          (ETH_DMADESCTypeDef *)((uint32_t)DMAPTPTxDescToSet + 0x10 +
                                 ((ETH->DMABMR & ETH_DMABMR_DSL) >> 2));
    }
  }
...
uint32_t ETH_HandlePTPRxPkt(uint8_t *ppkt, uint32_t *PTPRxTab) {
  ...
  if (((DMARxDescToGet->Status & ETH_DMARxDesc_ES) == (uint32_t)RESET) &&
    ...
    for (offset = 0; offset < framelength; offset++) {
      (*(ppkt + offset)) =
          (*(__IO uint8_t *)((DMAPTPRxDescToGet->Buffer1Addr) + offset));
    }
  } else {
    framelength = ETH_ERROR;
  }
  ...
  *PTPRxTab = DMARxDescToGet->Buffer2NextDescAddr;
  ...
  if ((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RCH) !=
      (uint32_t)RESET) {
    DMARxDescToGet =
        (ETH_DMADESCTypeDef *)(DMAPTPRxDescToGet->Buffer2NextDescAddr);
