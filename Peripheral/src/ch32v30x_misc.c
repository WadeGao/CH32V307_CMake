/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_misc.c Author :
 *WCH Version            : V1.0.0 Date               : 2021/06/06 Description :
 *This file provides all the miscellaneous firmware functions . Copyright (c)
 *2021 Nanjing Qinheng Microelectronics Co., Ltd. SPDX-License-Identifier:
 *Apache-2.0
 *********************************************************************************/
  ...
  if (NVIC_Priority_Group == NVIC_PriorityGroup_0) {
    ...
    if (NVIC_InitStruct->NVIC_IRQChannelPreemptionPriority == 1) {
      NVIC_SetPriority(
          ...
          (1 << 7) | (NVIC_InitStruct->NVIC_IRQChannelSubPriority << 4));
    } else {
      NVIC_SetPriority(
          NVIC_InitStruct->NVIC_IRQChannel,
          (0 << 7) | (NVIC_InitStruct->NVIC_IRQChannelSubPriority << 4));
    }
