/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_usart.c Author
 *: WCH Version            : V1.0.0 Date               : 2021/06/06 Description
 *: This file provides all the USART firmware functions. Copyright (c) 2021
...
#define CTLR3_ONEBITE_Set \
  ((uint16_t)0x0800) /* USART ONEBITE mode Enable Mask */
#define CTLR3_ONEBITE_Reset \
  ((uint16_t)0xF7FF) /* USART ONEBITE mode Disable Mask */
 ...
 * disabled. USART_IT_CTS - CTS change interrupt. USART_IT_LBD - LIN Break
 * detection interrupt. USART_IT_TXE - Transmit Data Register empty interrupt.
 ...
 * length. USART_LINBreakDetectLength_10b - 10-bit break detection.
 ...
 * @return  none
 */
void USART_SendBreak(USART_TypeDef *USARTx) { USARTx->CTLR1 |= CTLR1_SBK_Set; }
 ...
 *            USART_IT_RXNE - Receive Data register not empty interrupt.
 *            USART_IT_IDLE - Idle line detection interrupt.
 *            USART_IT_ORE_RX - OverRun Error interrupt if the RXNEIE bit is
 * set. USART_IT_ORE_ER - OverRun Error interrupt if the EIE bit is set.
