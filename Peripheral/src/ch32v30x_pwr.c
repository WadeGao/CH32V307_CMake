/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_pwr.c Author :
 *WCH Version            : V1.0.0 Date               : 2021/06/06 Description :
 *This file provides all the PWR firmware functions. Copyright (c) 2021 Nanjing
 ...
 *            PWR_Regulator_LowPower - STOP mode with regulator in low power
 * mode PWR_STOPEntry - specifies if STOP mode in entered with WFI or WFE
 * instruction. PWR_STOPEntry_WFI - enter STOP mode with WFI instruction
 ...
 * @return  none
 */
void PWR_ClearFlag(uint32_t PWR_FLAG) { PWR->CTLR |= PWR_FLAG << 2; }
 ...
 *
 * @brief   Enters STANDBY mode with RAM data retention function on (VBAT
 * Enable).
 *
 * @return  none
 */
 ...
 *
 * @brief   Enters STANDBY mode with RAM data retention function and LV mode
 * on(VBAT Enable).
 *
 * @return  none
 */
