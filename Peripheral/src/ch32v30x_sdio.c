/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_SDIO.c Author :
 *WCH Version            : V1.0.0 Date               : 2021/06/06 Description :
 *This file provides all the SDIO firmware functions. Copyright (c) 2021 Nanjing
 ...
 *
 * @param   DIO_IT - specifies the SDIO interrupt sources to be enabled or
 * disabled. SDIO_IT_CCRCFAIL SDIO_IT_DCRCFAIL SDIO_IT_CTIMEOUT SDIO_IT_DTIMEOUT
 *            SDIO_IT_TXUNDERR
 *            SDIO_IT_RXOVERR
 *            SDIO_IT_CMDREND
 *            SDIO_IT_CMDSENT
 ...
 *            SDIO_FLAG_DBCKEND - Data block sent/received (CRC check passed)
 *            SDIO_FLAG_SDIOIT - SD I/O interrupt received
 *            SDIO_FLAG_CEATAEND - CE-ATA command completion signal received for
 * CMD61
 *
 * @return  none
 */
void SDIO_ClearFlag(uint32_t SDIO_FLAG) { SDIO->ICR = SDIO_FLAG; }
 ...
 *            SDIO_IT_CCRCFAIL - Command response received (CRC check failed)
 * interrupt SDIO_IT_DCRCFAIL - Data block sent/received (CRC check failed)
 * interrupt SDIO_IT_CTIMEOUT - Command response timeout interrupt
 ...
 *            SDIO_IT_CMDREND - Command response received (CRC check passed)
 * interrupt SDIO_IT_CMDSENT - Command sent (no response required) interrupt
 *            SDIO_IT_DATAEND -  Data end (data counter, SDIDCOUNT, is zero)
 * interrupt SDIO_IT_STBITERR - Start bit not detected on all data signals in
 * wide bus mode interrupt SDIO_IT_DBCKEND - Data block sent/received (CRC check
 * passed) interrupt SDIO_IT_CMDACT - Command transfer in progress interrupt
 ...
 *            SDIO_IT_CEATAEND - CE-ATA command completion signal received for
 * CMD61 interrupt
 *
 ...
 *            SDIO_IT_DATAEND - Data end (data counter, SDIDCOUNT, is zero)
 * interrupt SDIO_IT_STBITERR - Start bit not detected on all data signals in
 * wide bus mode interrupt SDIO_IT_SDIOIT - SD I/O interrupt received interrupt
 *            SDIO_IT_CEATAEND - CE-ATA command completion signal received for
 * CMD61
 *
 * @return  RTC counter value
 */
void SDIO_ClearITPendingBit(uint32_t SDIO_IT) { SDIO->ICR = SDIO_IT; }
