/********************************** (C) COPYRIGHT
 ******************************** File Name          : ch32v30x_tim.h Author :
...
typedef struct {
  ...
  uint16_t TIM_OCIdleState; /* Specifies the TIM Output Compare pin state during
...
#define TIM_ICSelection_TRC                                                   \
  ((uint16_t)0x0003) /* TIM Input 1, 2, 3 or 4 is selected to be connected to \
                        TRC. */

/* TIM_Input_Capture_Prescaler */
#define TIM_ICPSC_DIV1                                                         \
  ((uint16_t)0x0000) /* Capture performed each time an edge is detected on the \
                        capture input. */
